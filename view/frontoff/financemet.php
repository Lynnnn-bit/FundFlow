...
<head>
    ...
    <link rel="stylesheet" href="css/styfinan.css">
...
<body>
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <select class="form-select" name="status" required>
                        <option value="en_attente" <?= ($editMode && $financeToEdit['status'] == 'en_attente') ? 'selected' : '' ?>>En attente</option>
                        <option value="approuve" <?= ($editMode && $financeToEdit['status'] == 'approuve') ? 'selected' : '' ?>>Approuvé</option>
                        <option value="rejete" <?= ($editMode && $financeToEdit['status'] == 'rejete') ? 'selected' : '' ?>>Rejeté</option>
                    </select>
                </div>
                    ...
                    <tbody>
                        <?php foreach ($existingDemands as $demand): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge <?= 
                                        $demand['status'] == 'approuve' ? 'bg-success' : 
    ...
    <script>
        // Slider functionality for amount input
        const amountInput = document.querySelector('input[name="montant"]');
        if (amountInput) {
            amountInput.addEventListener('input', function() {
                const value = parseInt(this.value).toLocaleString('fr-FR');
                this.nextElementSibling.textContent = value + ' €';
            });
        }

        // Slider functionality for duration input
        const durationInput = document.querySelector('input[name="duree"]');
        if (durationInput) {
            durationInput.addEventListener('input', function() {
                this.nextElementSibling.textContent = this.value + ' mois';
            });
        }

        // Initialize values
        if (amountInput && amountInput.nextElementSibling) {
            amountInput.nextElementSibling.textContent = 
                parseInt(amountInput.value).toLocaleString('fr-FR') + ' €';
        }
        if (durationInput && durationInput.nextElementSibling) {
            durationInput.nextElementSibling.textContent = durationInput.value + ' mois';
        }
    </script>
</body>
</html>