document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        ...
        form.addEventListener('submit', function(e) {
            ...
            const fields = [
                ...
                {name: 'logo', required: true, type: 'file', accept: 'image/*'},
                {name: 'video_presentation', required: true, type: 'file', accept: 'video/*'}
            ];
    ...
    function validateField(input, config = {}) {
        ...
        if (config.type === 'file') {
            ...
            } else if (config.accept && input.files[0]) {
                const fileType = input.files[0].type;
                const acceptedTypes = config.accept.split(',');
                if (!acceptedTypes.some(type => fileType.startsWith(type.replace('*', '')))) {
                    isValid = false;
